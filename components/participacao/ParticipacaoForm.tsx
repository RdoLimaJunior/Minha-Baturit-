...
import { TipoPublicacao } from '../../types';
...
const ParticipacaoForm: React.FC<ParticipacaoFormProps> = ({ goBack }) => {
            ...
            <div className="space-y-4">
                ...
                <Card>
                    <div className="text-center mb-6">
                        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Criar Publicação</h1>
                        <p className="text-slate-600 dark:text-slate-300 mt-1">O que você gostaria de compartilhar?</p>
                    </div>
                    <div className="space-y-3">
                        {Object.values(TipoPublicacao).map(type => (
                            <Card key={type} onClick={() => handleTypeSelect(type)} className="flex items-center space-x-4 text-left !p-4 border-2 border-transparent hover:border-indigo-500 dark:hover:border-indigo-600 hover:shadow-lg dark:hover:bg-slate-700">
                                <Icon name={TIPO_PUBLICACAO_METADATA[type].icon} className="text-3xl text-indigo-600 dark:text-indigo-400" />
                                <div>
                                    <h3 className="font-bold text-slate-800 dark:text-slate-100">{TIPO_PUBLICACAO_METADATA[type].title}</h3>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">{TIPO_PUBLICACAO_METADATA[type].description}</p>
                                </div>
                            </Card>
                        ))}
                    </div>
        ...
        <div className="space-y-4">
            ...
            <Card>
                <form onSubmit={handleSubmit} className="space-y-6">
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{TIPO_PUBLICACAO_METADATA[tipo!]?.title}</h1>
                    
                    <div>
                        <label htmlFor="title" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Título *</label>
                        <input id="title" type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Resuma em poucas palavras..." className="w-full p-2 bg-white text-slate-900 border border-slate-300 rounded-md focus:ring-indigo-600 focus:border-indigo-600 dark:bg-slate-700 dark:border-slate-600 dark:text-white" />
                    </div>

                    <div>
                        <label htmlFor="descricao" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Descrição *</label>
                        <textarea id="descricao" rows={5} value={descricao} onChange={e => setDescricao(e.target.value)} placeholder="Conte os detalhes que ajudam a entender..." className="w-full p-2 bg-white text-slate-900 border border-slate-300 rounded-md focus:ring-indigo-600 focus:border-indigo-600 dark:bg-slate-700 dark:border-slate-600 dark:text-white"></textarea>
                    </div>

                     <div>
                        <label htmlFor="bairro" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Bairro *</label>
                        <select id="bairro" value={bairro} onChange={(e) => setBairro(e.target.value)} className="w-full p-2 bg-white text-slate-900 border border-slate-300 rounded-md focus:ring-indigo-600 focus:border-indigo-600 dark:bg-slate-700 dark:border-slate-600 dark:text-white">
                    ...
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Fotos (Opcional, até {MAX_FOTOS})</label>
                        <div className="grid grid-cols-4 gap-2">
                            {Array.from({ length: MAX_FOTOS }).map((_, index) => {
                                ...
                                if (index === fotos.length) {
                                    return (
                                        <label key="add-photo" htmlFor="foto-upload" className="aspect-square w-full flex items-center justify-center rounded-md border-2 border-dashed border-slate-300 dark:border-slate-600 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700">
                                ...
                                return (
                                    <div key={`placeholder-${index}`} className="aspect-square w-full flex items-center justify-center rounded-md bg-slate-100 dark:bg-slate-700/50 border-2 border-dashed border-slate-200 dark:border-slate-600/50">
                                        <Icon name="image" className="text-3xl text-slate-300 dark:text-slate-500" />
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                        <div className="flex items-center justify-between">
                            <label htmlFor="anonymous" className="text-sm font-medium text-slate-700 dark:text-slate-300">Publicar como anônimo?</label>
                            <button type="button" onClick={() => setIsAnonymous(!isAnonymous)} className={`${isAnonymous ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-600'} relative inline-flex h-6 w-11 items-center rounded-full transition-colors`}>
                        ...
                        <div className="flex items-start space-x-2">
                            ...
                            <label htmlFor="lgpd" className="text-xs text-slate-500 dark:text-slate-400">
