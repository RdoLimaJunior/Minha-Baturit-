...
const ResultItem: React.FC<{ icon: string, title: string, subtitle: string, onClick: () => void }> = ({ icon, title, subtitle, onClick }) => (
    <div onClick={onClick} className="p-4 flex items-center space-x-4 cursor-pointer hover:bg-gray-100 rounded-lg">
        <Icon name={icon} className="text-2xl text-gray-500" />
        <div>
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center space-x-2">
                ...
                <h2 className="text-2xl font-bold text-gray-800">Busca Global</h2>
            </div>
            ...
            <div className="sticky top-[70px] bg-gray-50 py-2 z-5">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full p-4 pl-12 bg-white text-gray-900 border border-gray-300 rounded-full focus:ring-indigo-600 focus:border-indigo-600 text-base"
                        autoFocus
                    />
                </div>
            </div>
            ...
            {isLoading && (
                <Card>
                    <div className="space-y-4">
                        {[...Array(2)].map((_, i) => (
                            <section key={i}>
                                <div className="h-6 w-1/4 bg-gray-100 rounded-md my-2 animate-pulse"></div>
                                <ResultSkeletonItem />
                                <ResultSkeletonItem />
                            </section>
                        ))}
            ...
            {!isLoading && searchResults && (
                <Card>
                    <h3 className="font-bold text-gray-800 mb-2 px-4">
                        {totalResults} {totalResults === 1 ? 'resultado encontrado' : 'resultados encontrados'}
                    </h3>
                    {searchResults.protocolos.length > 0 && (
                        <section>
                            <h4 className="font-semibold text-sm text-gray-500 bg-gray-100 p-2 rounded-md my-2">Protocolos</h4>
                    ...
                    {searchResults.noticias.length > 0 && (
                        <section>
                            <h4 className="font-semibold text-sm text-gray-500 bg-gray-100 p-2 rounded-md my-2">Notícias</h4>
                    ...
                    {searchResults.turismo.length > 0 && (
                        <section>
                            <h4 className="font-semibold text-sm text-gray-500 bg-gray-100 p-2 rounded-md my-2">Turismo</h4>
                    ...
                    {searchResults.servicos.length > 0 && (
                        <section>
                            <h4 className="font-semibold text-sm text-gray-500 bg-gray-100 p-2 rounded-md my-2">Serviços Online</h4>
            ...
            {!isLoading && !searchResults && (
                <Card className="text-center py-10">
                    <Icon name="search" className="text-5xl text-gray-400 mx-auto" />
                    <p className="text-gray-600 mt-4">Digite pelo menos 3 caracteres para iniciar a busca.</p>
                </Card>
            )}

            {!isLoading && searchResults && totalResults === 0 && (
                 <Card className="text-center py-10">
                    <Icon name="search_off" className="text-5xl text-gray-400 mx-auto" />
                    <p className="text-gray-600 mt-4">Nenhum resultado encontrado para "<span className="font-semibold">{searchTerm}</span>".</p>
