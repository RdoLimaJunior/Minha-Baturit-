...
const SecretariaItem: React.FC<{ secretaria: Secretaria }> = ({ secretaria }) => (
  <Card className="flex flex-col items-center text-center">
    <img 
      ...
      className="w-24 h-24 rounded-full object-cover mb-4 border-4 border-slate-100 shadow-md"
    />
    <h3 className="text-xl font-bold text-slate-800">{secretaria.nome}</h3>
    <div className="mt-2">
        <p className="text-md font-semibold text-slate-700 flex items-center justify-center space-x-1">
            <span>{secretaria.secretario}</span>
            <Icon name="check_circle" className="text-green-500 text-base" />
        </p>
        <p className="text-sm text-slate-500">{secretaria.cargo}</p>
    </div>
    
    <div className="mt-6 w-full text-left space-y-3">
        <div className="flex items-start space-x-3 text-slate-700">
            <Icon name="email" className="text-xl text-slate-400 mt-0.5" />
        ...
        <div className="flex items-start space-x-3 text-slate-700">
            <Icon name="schedule" className="text-xl text-slate-400 mt-0.5" />
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center space-x-2">
        ...
        <h2 className="text-2xl font-bold text-slate-800">Secretarias</h2>
      </div>
      ...
      {loading ? (
        ...
        <Card className="text-center">
          <p className="text-slate-600">Nenhuma secretaria encontrada.</p>
        </Card>
      )}
    </div>
  );
