...
import { MOCK_TURISMO_ITENS, MOCK_PREDIOS_PUBLICOS } from '../constants';
...
const generalSuggestions = [
    ...
    `Já conhece o ${getRandomItem(MOCK_PREDIOS_PUBLICOS.filter(p => p.categoria === 'Educação' || p.categoria === 'Assistência Social')).nome}? Vale a pena conferir os serviços.`,
    `Que tal visitar o ${getRandomItem(MOCK_TURISMO_ITENS.filter(p => p.categoria === 'Pontos Turísticos')).nome}? É um dos cartões-postais da nossa cidade.`,
];
